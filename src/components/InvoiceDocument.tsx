...
<body>
  ...
  const handleDownloadPng = async () => {
    ...
    try {
      ...
      await downloadFile(a4Canvas.toDataURL("image/png"), `Invoice-${invoice?.invoice_number}.png`);
    } catch (error) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      await downloadFile(pdf.output("blob"), `Invoice-${invoice?.invoice_number}.pdf`);
    } catch (error) {
